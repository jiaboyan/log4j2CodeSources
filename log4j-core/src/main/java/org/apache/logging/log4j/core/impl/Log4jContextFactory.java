...
//   初始化context选择器：从 log4j2.component.properties配置文件中查找 Log4jContextSelector 的值：
    public Log4jContextFactory() {
        //创建context选择器，一般情况下为ClassLoaderContextSelector。  createShutdownCallbackRegistry没看懂
    ...
    //创建context选择器：从log4j2.component.properties文件中 查找 Log4jContextSelector 为key的值：
    private static ContextSelector createContextSelector() {
        try {
            //从配置文件中查找：并初始化   异步logger时候最好指定此值
...
//        在不适用异步logger的情况下，返回ClassLoaderContextSelector
    ...
    public LoggerContext getContext(final String fqcn, final ClassLoader loader, final Object externalContext,
        ...
        //从选择器中，获取logContext对象：
        final LoggerContext ctx = selector.getContext(fqcn, loader, currentContext);
        if (externalContext != null && ctx.getExternalContext() == null) {
            ctx.setExternalContext(externalContext);
        }
        //判断loggerContext状态：初始化完成，但是还没开始读取log4j2.xml：
