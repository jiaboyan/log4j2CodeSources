...
public class LogManager {
     ...
     */
    static {
        //classpath下 查找 log4j2.component.properties 文件：
        final PropertiesUtil managerProps = PropertiesUtil.getProperties();
        //在配置文件中查找 log4j2.loggerContextFactory为key 的值：
        ...
        if (factory == null) {
            ...
            if (ProviderUtil.hasProviders()) {
                ...
                for (final Provider provider : ProviderUtil.getProviders()) {
                    //通过Provider创建LoggerContextFactory对象，具体的class就配置在META-INF/log4j-provider.properties文件中
                    final Class<? extends LoggerContextFactory> factoryClass = provider.loadLoggerContextFactory();
                    //如果Class不为null ：
                    if (factoryClass != null) {
                        try {
                            //添加到 LoggerContextFactory  map中，优先级为key，LoggerContextFactory对象为 value
    ...
    public static Logger getLogger(final Class<?> clazz) {
        ...
        //先获取loggerContext，再通过loggerContext获取logger对象：
